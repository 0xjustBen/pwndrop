...
func SubFileCreateHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Warning("audit user=- action=subfile_upload status=unauthorized machine=%q method=%s path=%s ip=%s",
	...
	if err != nil {
		log.Warning("audit user=%d action=subfile_upload status=failure machine=%q err=%v ip=%s",
	...
	if err := SaveUploadedFile(file, fhead, save_path); err != nil {
		log.Warning("audit user=%d action=subfile_upload status=failure file=%q machine=%q err=%v ip=%s",
	...
	AuditEvent(uid, "subfile_upload", "success", name, machineName, r)
